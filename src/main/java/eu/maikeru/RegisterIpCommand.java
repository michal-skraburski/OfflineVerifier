...
@NullMarked
public class RegisterIpCommand implements BasicCommand {
    ...
    public void execute(CommandSourceStack source, String[] args) {
        ...
        if (pred != null && lply == null) {
                if (!pred.getIpsCopy().contains(player.getAddress().getHostString())) {
                    pred.trustIp(player.getAddress().getHostString());
                    source.getSender().sendMessage(Component.text("Success! You have saved this ip address.", NamedTextColor.GREEN));
                }else {
                    source.getExecutor().sendMessage(Component.text("Your ip is already registered!", NamedTextColor.RED));
                }
        }else {
